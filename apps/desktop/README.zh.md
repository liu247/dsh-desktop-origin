...
- **窗口**：关闭即隐藏到托盘；托盘菜单负责显示与退出。退出前先停止服务进程。macOS 的 Cmd+Q 与系统「退出」菜单经由 `ExitRequested` 事件执行同样的清理，任何退出路径都不会遗留正在运行的 web 服务。
- **残留服务清理**：启动时壳会回收被强杀遗留的服务。已记录的 pid 文件是来源之一；文件缺失时，壳会扫描桌面端口，杀掉命令行匹配所 spawn 的 `dsh web` 服务的进程——强杀不会卡住端口，也不会让下一次启动一直停在加载页转圈。
